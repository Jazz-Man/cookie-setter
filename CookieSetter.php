...
class CookieSetter
{
     ...
     * @var bool[]
     */
    private static $_is_browser_compatible = [];
        ...
        if (1 === \version_compare('7.3.0', \PHP_VERSION)) {
            ...
            if (self::isBrowserSameSiteCompatible($user_agent)) {
                ...
                foreach ($headers_list as $_header) {
                    if (!$is_modified && 0 === \strpos($_header, 'Set-Cookie: '.$name)) {
                        $additional_labels = [];

                        $is_secure = ('None' === $same_site ? true : $is_secure);

                        $new_label = '; HttpOnly';
                        if ($is_httponly && false === \strpos($_header, $new_label)) {
                            $additional_labels[] = $new_label;
                        }

                        $new_label = '; Secure';
                        if ($is_secure && false === \strpos($_header, $new_label)) {
                            $additional_labels[] = $new_label;
                        }

                        $new_label = '; SameSite='.$same_site;
                        if (false === \strpos($_header, $new_label)) {
                            $additional_labels[] = $new_label;
                        }
    ...
    private static function _setIsBrowserCompatible(string $user_agent_key, bool $value)
    {
